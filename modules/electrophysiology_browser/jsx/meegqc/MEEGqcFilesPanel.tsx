...
function MEEGqcFilesPanel({
  ...
  const {reportFeatureAvailability} = useContext(ImagingGatewayContext);
    ...
    const fetchMEEGqcFiles = async () => {
      ...
      try {
        ...
        if (response.status === 404) {
          if (!ignoreResponse) {
            reportFeatureAvailability(physioFileID, 'meegqc', false);
          }
          return;
        }
        ...
        if (!ignoreResponse) {
          reportFeatureAvailability(physioFileID, 'meegqc', true);
  ...
  }, [physioFileID, reportFeatureAvailability]);
